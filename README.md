...
```shell
# Example
docker build -t xml-lint:php-8.5 --build-arg=PHP_VERSION="8.5" .

# PHP_VERSION: choose between 8.1, 8.2, 8.3, 8.4 and  8.5
...
docker build -t xml-lint:php-8.5 --build-arg=PHP_VERSION="8.5" --build-arg=PHP_CS_FIXER=true .

# Use this image to run xml-lint:
cd tests/functional/_testdata
docker run -it --rm -v "$PWD":/var/src -w /var/src xml-lint:php-8.5 -r -v -- ./
```
